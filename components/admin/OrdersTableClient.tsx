...
export default function OrdersTableClient({
  ...
  const groupedByMonth = orders.reduce<Record<string, MonthGroup>>(
    (acc, order) => {
  ...
  const reportData = Object.values(groupedByMonth).sort((a, b) => {
  ...
  const exportToExcel = (monthData: MonthGroup) => {
    const data = monthData.orders.map((order) => ({
      "No. Order": order.id,
    ...
    // Add total row
    // (Status kolom diset "pending" biar type tetap valid)
    data.push({
      "No. Order": "TOTAL",
      Pelanggan: "",
      WhatsApp: "",
    ...
    XLSX.writeFile(
      workbook,
      `Laporan-NanaBites-${monthData.name.replace(" ", "-")}.xlsx`,
    );
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "pending":
        return (
          <span className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 w-max">
            <Clock className="w-3 h-3" /> Pending
          </span>
        );
      case "processing":
        return (
          <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 w-max">
            <Clock className="w-3 h-3" /> Diproses
          </span>
        );
      case "completed":
        return (
          <span className="bg-green-100 text-green-800 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1 w-max">
  ...
  return (
    <div className="pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
        <div>
          <h1 className="font-playfair text-4xl font-bold text-pink-900 mb-2">
            Manajemen Pesanan 🍱
          </h1>
          <p className="text-pink-600/60 font-medium">
            Kelola pesanan masuk dan pantau omzet kamu kak.
          </p>
        </div>
        ...
        <>
          <div className="flex bg-white rounded-2xl shadow-sm border border-pink-100 p-1 mb-6 w-max">
            <button
              ...
              className={`px-5 py-2 text-xs font-bold rounded-xl transition-colors ${filter === "all" ? "bg-pink-100 text-pink-700" : "text-gray-400 hover:bg-pink-50"}`}>
              Semua
            </button>
            <button
              onClick={() => setFilter("pending")}
              className={`px-5 py-2 text-xs font-bold rounded-xl transition-colors ${filter === "pending" ? "bg-pink-100 text-pink-700" : "text-gray-400 hover:bg-pink-50"}`}>
              Pending
            </button>
            <button
              onClick={() => setFilter("processing")}
              className={`px-5 py-2 text-xs font-bold rounded-xl transition-colors ${filter === "processing" ? "bg-pink-100 text-pink-700" : "text-gray-400 hover:bg-pink-50"}`}>
              Diproses
            </button>
            <button
              onClick={() => setFilter("completed")}
              className={`px-5 py-2 text-xs font-bold rounded-xl transition-colors ${filter === "completed" ? "bg-pink-100 text-pink-700" : "text-gray-400 hover:bg-pink-50"}`}>
              Selesai
            </button>
          </div>
                ...
                <tbody className="divide-y divide-pink-50">
                  {filteredOrders.map((order) => (
                    <tr
                      ...
                      <td className="p-5 font-bold text-pink-900">
                        #{order.id}
                      </td>
                      <td className="p-5">
              ...
              <div className="p-6 bg-pink-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  ...
                  <p className="text-pink-600/70 text-sm font-medium">
                    {month.orders.length} Pesanan Berhasil
                  </p>
                </div>
                  ...
                  <tbody className="divide-y divide-pink-50">
                    {month.orders.map((order) => (
                      <tr
                        ...
                        <td className="p-5">{getStatusBadge(order.status)}</td>
                        <td className="p-5 text-right font-bold text-pink-600">
                          {formatRupiah(order.totalPrice)}
                        </td>
                      </tr>
                    ))}
