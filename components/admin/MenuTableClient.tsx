...
import { Edit2, Trash2, Plus, Star, Eye, EyeOff, Upload, X, ImageIcon } from "lucide-react";
...
export default function MenuTableClient({ initialMenu }: { initialMenu: any[] }) {
  ...
  const toggleStatus = async (
    ...
    try {
      ...
      if (!res.ok) throw new Error();
      setMenu(menu.map((m) => (m.id === id ? { ...m, [field]: !currentValue } : m)));
  ...
  const getStatusBadge = (item: any) => {
    if (item.isComingSoon) return <span className="text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-bold">Coming Soon</span>;
    if (!item.isAvailable) return <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-bold">Habis</span>;
    return <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-bold">Tersedia</span>;
  };
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="font-playfair text-3xl font-bold text-pink-900">Manajemen Menu 🍱</h1>
        <button
                ...
                <tr key={item.id} className="hover:bg-pink-50/30 transition-colors">
                  <td className="p-4">
                    <div className="w-12 h-12 rounded-lg overflow-hidden bg-pink-100 relative">
                      <Image
                        src={item.image || `https://placehold.co/100x100/FDF2F8/BE185D?text=${item.name[0]}`}
                        alt={item.name}
                        fill
                  ...
                  <td className="p-4">
                    ...
                    <p className="text-xs text-gray-500 line-clamp-1 max-w-[180px] mt-0.5">{item.description}</p>
                    <div className="mt-1">{getStatusBadge(item)}</div>
                  </td>
                  <td className="p-4 font-medium text-pink-600">{formatRupiah(item.price)}</td>
                  ...
                  <td className="p-4 text-center">
                    <button
                      onClick={() => toggleStatus(item.id, "isBestSeller", item.isBestSeller)}
                    ...
                    >
                      <Star className={`w-5 h-5 ${item.isBestSeller ? "fill-current" : ""}`} />
                    </button>
                  </td>
                  ...
                  <td className="p-4 text-center">
                    <button
                      onClick={() => toggleStatus(item.id, "isComingSoon", item.isComingSoon ?? false)}
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button onClick={() => openDeleteModal(item)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-pink-950/30 backdrop-blur-sm" onClick={() => setIsModalOpen(false)} />
          <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="bg-pink-50 px-6 py-5 border-b border-pink-100 flex items-center justify-between flex-shrink-0">
              ...
              <button onClick={() => setIsModalOpen(false)} className="text-pink-400 hover:text-pink-600 p-1 rounded-lg hover:bg-pink-100">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="overflow-y-auto flex-1">
              <div className="p-6 space-y-5">
                ...
                {/* Nama */}
                <div>
                  <label className="block text-sm font-semibold text-pink-900 mb-1.5">Nama Menu <span className="text-pink-500">*</span></label>
                  <input
                    required
                    type="text"
                    value={formData.name}
                ...
                {/* Deskripsi */}
                <div>
                  <label className="block text-sm font-semibold text-pink-900 mb-1.5">Deskripsi <span className="text-pink-500">*</span></label>
                  <textarea
                    required
                ...
                {/* Harga */}
                <div>
                  <label className="block text-sm font-semibold text-pink-900 mb-1.5">Harga (Rp) <span className="text-pink-500">*</span></label>
                  <input
                    required
                    type="number"
                    min="0"
                ...
                {/* Upload Gambar */}
                <div>
                  <label className="block text-sm font-semibold text-pink-900 mb-1.5">Gambar Menu</label>
                  ...
                  {imagePreview ? (
                    ...
                    >
                      ...
                      <p className="text-sm text-pink-400 font-medium">Klik untuk upload gambar</p>
                      <p className="text-xs text-pink-300">PNG, JPG, WEBP — maks 2MB</p>
                    </div>
                  )}
                  ...
                  {/* Atau URL */}
                  <div className="mt-3">
                    <p className="text-xs text-gray-400 mb-1.5">atau pakai URL gambar:</p>
                    <input
                      type="text"
                ...
                <div className="bg-pink-50/50 rounded-2xl p-4 border border-pink-100 space-y-3">
                  <p className="text-sm font-semibold text-pink-900 mb-2">Status & Label</p>

                  <label className="flex items-center justify-between cursor-pointer">
                    <div>
                      <p className="text-sm font-medium text-gray-900">Tersedia</p>
                      <p className="text-xs text-gray-500">Menu muncul dan bisa dipesan pelanggan</p>
                    </div>
                    <div
                    ...
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${formData.isAvailable ? "translate-x-6" : "translate-x-1"}`} />
                    </div>
                  </label>
                  ...
                  <label className="flex items-center justify-between cursor-pointer">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Tampilkan badge Best Seller di kartu menu</p>
                    </div>
                    <div
                    ...
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${formData.isBestSeller ? "translate-x-6" : "translate-x-1"}`} />
                    </div>
                  </label>
                  ...
                  <label className="flex items-center justify-between cursor-pointer">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Tampilkan badge Coming Soon, tombol dinonaktifkan</p>
                    </div>
                    <div
                    ...
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${formData.isComingSoon ? "translate-x-6" : "translate-x-1"}`} />
                    </div>
                  </label>
                </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-pink-950/40 backdrop-blur-sm" onClick={() => setIsDeleteModalOpen(false)} />
          <div className="relative w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden p-6 text-center">
            ...
            <h3 className="font-heading text-xl font-bold text-gray-900 mb-2">Hapus Menu?</h3>
            <p className="text-gray-500 text-sm mb-6">
              Kamu yakin mau menghapus <span className="font-bold text-pink-900">&quot;{itemToDelete?.name}&quot;</span>? Tindakan ini tidak bisa dibatalkan.
            </p>
