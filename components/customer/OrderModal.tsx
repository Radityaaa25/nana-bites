            ...
            {step === 'success' ? (
              ...
              <>
                <div className="bg-pink-50 p-8 text-center border-b border-pink-100">
                  ...
                  <h2 className="font-playfair text-2xl font-bold text-pink-900 mb-2">Yay! Pesanan Berhasil! 🎉</h2>
                  <p className="text-pink-600 text-sm">Terima kasih kak {successData?.order?.customerName}!</p>
                </div>

                <div className="p-6 max-h-[60vh] overflow-y-auto space-y-4">
                  <div className="flex justify-between items-center bg-pink-50/50 p-4 rounded-xl border border-pink-100">
                    <div>
                      <p className="text-xs text-pink-500 font-medium mb-1">No. Pesanan</p>
                      <p className="font-bold text-pink-900 text-lg">{successData?.id}</p>
                    </div>
                    <span className="bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-xs font-bold">⏳ Pending</span>
                  </div>

                  <div className="space-y-2">
                    <h3 className="font-bold text-pink-900 text-sm">📋 Ringkasan Pesanan</h3>
                    <div className="bg-pink-50/50 rounded-xl p-4 border border-pink-100 space-y-3">
                      {successData?.order?.items?.map((item: any, i: number) => (
                        <div key={i} className="flex justify-between text-sm">
                          <span className="text-pink-800">{item.quantity}x {item.name}</span>
                          <span className="font-medium text-pink-900">{formatRupiah(item.price * item.quantity)}</span>
                        </div>
                      ))}
                      <div className="pt-2 border-t border-pink-200 flex justify-between font-bold text-pink-900">
                        <span>Total</span>
                        <span className="text-pink-600">{formatRupiah(successData?.order?.totalPrice)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-sm bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-800">{successData?.order?.createdAt ? formatDate(successData.order.createdAt) : '-'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Pembayaran</span>
                      <span className="font-medium text-gray-800 uppercase">{successData?.order?.paymentMethod}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Pengambilan</span>
                      <span className="font-medium text-gray-800 uppercase">{successData?.order?.deliveryMethod}</span>
                    </div>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="bg-pink-50 p-6 flex items-center justify-between">
                  {step === 'payment' ? (
                    <button onClick={() => setStep('form')} className="text-pink-600 hover:text-pink-800">
                  ...
                  <h2 className="font-playfair text-xl font-bold text-pink-900 text-center flex-1">
                    {step === 'form' ? "Detail Pesanan" : "Konfirmasi Pembayaran"}
                  </h2>
                ...
                <div className="flex px-8 py-4 bg-white border-b border-pink-50">
                  <div className="flex-1 flex flex-col items-center gap-1">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === 'form' || step === 'payment' ? 'bg-pink-500 text-white' : 'bg-pink-100 text-pink-400'}`}>1</div>
                  ...
                  <div className="flex-1 flex flex-col items-center gap-1">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === 'payment' ? 'bg-pink-500 text-white' : 'bg-pink-100 text-pink-400'}`}>2</div>
